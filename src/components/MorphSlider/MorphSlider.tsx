...
type OglGl = (WebGL2RenderingContext | WebGLRenderingContext) & {
  renderer: Renderer;
  canvas: HTMLCanvasElement;
};

function makeFallbackTexture(gl: OglGl) {
  const size = 4;
...
class MorphEngine {
  ...
  raf: number;
    ...
    this.onContextLostCallback = onContextLostCallback;
  ...
  goTo(dir: number) {
    ...
    // 防御性清理：先 kill 旧 tween，避免并发动画导致抽搐
    if (this.tween) {
      this.tween.kill();
      this.tween = null;
    }
  ...
  endDrag() {
    ...
    // 防御性清理：先 kill 旧 tween，避免与 goTo / 上一次 endDrag 冲突
    if (this.tween) {
      this.tween.kill();
      this.tween = null;
    }
...
export default function MorphSlider({
  ...
  className = '',
  ...props
}: MorphSliderProps & React.HTMLAttributes<HTMLDivElement>) {
  ...
  const optsRef = useRef<MorphEngineOptions>(null as unknown as MorphEngineOptions);
  optsRef.current = {
    transition,
    duration,
    ease,
  ...
  // 延迟 + 可见性感知初始化：确保容器有真实尺寸后再创建 WebGL 上下文
  useEffect(() => {
    ...
    const initEngine = () => {
      ...
      onContextLost: () => setWebglFailed(true),
        ...
        // 初始化后立即再 resize 一次，兜底动画过渡中漏触发的情况
        ...
        logger.warn('[MorphSlider] WebGL init failed, falling back to plain image:', String(err));
        setWebglFailed(true);
      }
    };
    ...
    if (typeof IntersectionObserver !== 'undefined') {
      ...
      // 不支持 IntersectionObserver 时，延迟 100ms 后初始化
  ...
  const handleNext = useCallback(() => {
    ...
    // 手动翻页后触发 interacting 变化，重置 autoplay 计时
    setInteracting((v) => v);
  }, []);
  ...
  useEffect(() => {
    if (!autoplay || hovering || interacting) return undefined;
    const id = setTimeout(
      () => engineRef.current?.next(),
      Math.max(autoplayDelay, 1) * 1000,
    );
    return () => clearTimeout(id);
  }, [autoplay, autoplayDelay, hovering, index, interacting]);
  ...
  useEffect(() => {
    ...
    const onUp = () => {
      ...
      // 拖拽结束后延迟一帧再恢复 autoplay，避免与 endDrag 补间动画抢时间
